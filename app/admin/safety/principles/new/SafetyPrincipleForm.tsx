...
export default function SafetyPrincipleForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/admin/safety/principles", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to create safety principle.");
      }
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to create safety principle.",
      );
      setSaving(false);
    }
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Describe this safety principle..."
          required
          rows={6}
