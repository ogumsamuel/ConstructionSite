...
export default function SafetyStageEditForm({ stage }: Props) {
  const router = useRouter();

  const [title, setTitle] = useState(stage.title);
  const [description, setDescription] = useState(stage.description);
  const [displayOrder, setDisplayOrder] = useState(
    String(stage.display_order),
  );
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`/api/admin/safety/stages/${stage.id}`, {
        method: "PATCH",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to update project stage.");
      }
