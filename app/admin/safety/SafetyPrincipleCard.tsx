...
export default function SafetyPrincipleCard({ principle }: Props) {
  ...
  async function handleToggle() {
    ...
    try {
      ...
      if (!response.ok) {
        alert(result.error || "Failed to update principle.");
        return;
      }
  ...
  async function handleDelete() {
    const confirmed = window.confirm(
      `Are you sure you want to delete "${principle.title}"?`,
    );
    ...
    try {
      ...
      if (!response.ok) {
        alert(result.error || "Failed to delete principle.");
        return;
      }
    ...
    <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-5">
        ...
        <div className="flex gap-2">
          <Link
            href={`/admin/safety/principles/${principle.id}`}
